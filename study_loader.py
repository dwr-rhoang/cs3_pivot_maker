...
# Scenario management
Study = namedtuple(
    "Scenario", ["dv_path", "sv_path", "alias", "assumptions", "climate", "color"]
)


def load_studies_from_ledger(
    ledger_path: Path | str = "config/study_ledger.yaml",
) -> list[Study]:
...
def main(
    ledger_path="config/study_ledger.yaml", config_dir="config", append: bool = False
):
